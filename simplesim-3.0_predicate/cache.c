...
cache_create(char *name,		/* name of the cache */
	     ...
	     struct cache_t* cache_upper /* INCLUSION */ )
{
  struct cache_t *cp;
  struct cache_blk_t *blk;
  int i, j, bindex;
...
/* Martin Dimitrov
 * Verify the the inclusion property holds.
 */
void cache_verify_inclusion(struct cache_t *cp_l1, /* L1 cache */
			struct cache_t *cp_l2 /* L2 cache */)
{

	/* Loop though each set and each block block in the L1 cache,
	 * probe the L2 cache to verify that it is there - inclusion */
			...
			for(hindex=0; hindex<num_hash_buckets; hindex++){
				 ...
				 blk=blk->hash_next)
				{
					...
					if(blk->status & CACHE_BLK_VALID){
						...
						if(there == 0){
    						fatal("Inclusion does not hold for `%x' ", addr);
						}
					}
		 ...
		 tick_t now)		/* time of cache flush */
{
  ...
  if (blk)
      ...
      if (blk->status & CACHE_BLK_DIRTY)
	{
	  /* write back the invalidated block */
          cp->writebacks++;
	  /* commented out because of simulator bug with inclusion
	  lat += cp->blk_access_fn(Write,
